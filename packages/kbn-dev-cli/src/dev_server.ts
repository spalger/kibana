...
export class DevServer {
  ...
  run$ = new Rx.Observable<void>((subscriber) => {
    ...
    // start a server that will receive connection
    const connection$ = new Rx.Subject<Socket>();
    const server = createServer({ pauseOnConnect: true });
    subscriber.add(Rx.fromEvent<Socket>(server, 'connection').subscribe(connection$));
    server.listen(5601, () => {
      subscriber.add(() => server.close());
    });

    // buffer connections while the server is not ready
    const socketBuffer = new SocketBuffer(connection$);
    subscriber.add(socketBuffer.subscribe(subscriber));
    subscriber.add(
    ...
    const runServer = () =>
      usingServerProcess(this.script, this.argv, (proc) => {
        ...
        const socket$ = socketBuffer.socket$.pipe(
          tap((socket) => {
            proc.send(['SERVER_CONNECTION'], socket);
          })
        );
        ...
        return Rx.merge(log$, exit$, error$, socket$, msg$, triggerGracefulShutdown$);
      });
